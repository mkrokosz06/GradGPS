...
(ETI junk rows, PHYS 211/250, MATH 250/251, choose_credits option groups)
without creating matthew-test-001.
...
if __name__ == "__main__":
    patch_eti_catalog()
    # patch_eti_select_groups() was removed: it worked around the scraper
    # merging ETI's pools into one, which `pool_seq` now fixes properly. Its
    # guard fired on any unpaired choose_credits row, so keeping it would
    # re-merge the corrected pools on every run.
    patch_phys_alternatives()
    patch_known_alternatives()
    patch_choose_credits_option_groups()
    fix_junk_titles()
